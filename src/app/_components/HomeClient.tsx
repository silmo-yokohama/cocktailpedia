...
function FilterBadges({
  ...
  if (filters.base) {
    badges.push({ label: "ベース", value: getLabel(BASE_OPTIONS, filters.base) })
  }
  if (filters.technique) {
    badges.push({ label: "技法", value: getLabel(TECHNIQUE_OPTIONS, filters.technique) })
  }
  if (filters.glass) {
    badges.push({ label: "グラス", value: getLabel(GLASS_OPTIONS, filters.glass) })
  }
  if (filters.temperature) {
    badges.push({ label: "冷たさ", value: getLabel(TEMPERATURE_OPTIONS, filters.temperature) })
  }
  if (filters.carbonation) {
    badges.push({ label: "炭酸", value: getLabel(CARBONATION_OPTIONS, filters.carbonation) })
  }
  if (filters.color) {
    badges.push({ label: "カラー", value: getLabel(COLOR_OPTIONS, filters.color) })
  }
  if (filters.alcoholStrength) {
    badges.push({ label: "度数", value: getLabel(ALCOHOL_STRENGTH_OPTIONS, filters.alcoholStrength) })
  }
