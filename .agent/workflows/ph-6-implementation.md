...
```
Step1: 全体実装計画を立てる【AG + 施主】
  - AGが要件・設計を確認し、タスク分割を提案
  - 施主と議論しながら全体計画を確定
  - implementation_plan.md を作成
  ↓
Step2: 詳細実装計画サイクル【AG + 施主 → CC】
  - AGがタスクごとの詳細計画（task-XX-xxx.md）を作成
  - 施主とAGでレビュー → 承認
  - 施主がCCに実装依頼
  - CCが実装 → 施主確認 → PR作成
  - AGがPRレビュー → マージ
...
タスク分割が確定したら、AGが全体実装計画書（`implementation_plan.md`）を作成する。

### Step1 完了条件

- [ ] AGがドキュメントを確認している
- [ ] タスク分割が施主と合意されている
- [ ] **全体実装計画書（implementation_plan.md）が作成されている**
- [ ] 施主が「この計画で進めよう」と承認
...
全体計画に基づき、タスクごとに以下のサイクルを回す。

```
AG: task-XX-xxx.md（詳細実装計画）を作成
  ↓
施主 + AG: レビュー・承認
  ↓
施主: CCに実装依頼（詳細計画を渡す）
...
CC: PR作成
  ↓
AG: PRレビュー（詳細計画との整合性、要件・設計からの乖離をチェック）
  ↓
施主: マージ
  ↓
次のタスクへ
...
AGは各タスクの詳細実装計画書を `docs/06_implementation/task-XX-xxx.md` に作成する。

**テンプレート**: `docs/_templates/task-detail.tmp.md` を使用
...
- 守るべき制約
- 参照ドキュメント
- Git運用ルール（ブランチ命名、PR作成タイミング）
- デザイン更新の有無（下記参照）

### 2-2. デザイン更新有無の判断【AG】

AGは作業内容からデザイン更新の有無を判断し、詳細計画書に反映する。

**デザイン更新あり（UI実装が含まれる場合）**:
```markdown
> [!IMPORTANT]
> **このタスクにはUI実装が含まれます。**
...
**デザイン更新なし（API、DB操作のみなど）**:
→ 上記の記載を省略する（トークン節約）

### 2-3. Git運用ルール【詳細計画書に記載】
...
**レビュー観点**:
- 詳細実装計画書との整合性
- 要件・設計から大きく乖離した進め方をしていないか
- 指摘事項はPRのディスカッションにコメント

**実行方法**: `mcp_github_get_pull_request_files` 等でPR内容を確認し、問題があれば `mcp_github_add_issue_comment` でコメント
